...
def run(
    ...
    for path, im, im0s, vid_cap, s in dataset:
        ...
        for i, det in enumerate(pred):  # per image
            ...
            if len(det):
                for c in det[:, 5].unique():  #label = 0 no-mask
                    if c == 0:
                        count += 1
                        if count > 5:
                            count = 0
                            if time.time() - tplay > 2:  # 防止声音叠加 2s播放完再继续
                                os.system(
                                'start /b ffmpeg/bin/ffplay.exe -autoexit -nodisp ffmpeg/2.mp3')  # 音乐播放
...
def voice():
    ...
    # if flag == 1:
        # mixer.init()
        # mixer.music.load('runs/1.mp3')
        # mixer.music.play()
        # time.sleep(5)
        # mixer.music.stop()
    from playsound import playsound
    playsound('runs/1.mp3')                        


def run_webcam(save_path='runs/detect', img_size=640, stride=32, augment=False, visualize=False,device=''):

    weights = r'runs/train/exp/weights/best.pt'
